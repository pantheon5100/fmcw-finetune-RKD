...
from model.model_shufflenet import Shuffle
from model.model_mobile import Mobile
from model.model_resnet import ResFT
...
def main():
    ...
    for i in range(6):
        ...
        for name in ["zk", "lt", "dl", "jc", "lg"]:
            train_data.append(Train_data[name][0][20*i:20*i+add_other_number])
            train_label.append(Train_data[name][1][20*i:20*i+add_other_number])
            val_5p_data.append(Train_data[name][0][20*i+4:20*(i+add_other_number)])
            val_5p_label.append(Train_data[name][1][20*i+4:20*(i+add_other_number)])
    ...
    val_5p_data = np.vstack(val_5p_data)
    val_5p_label = np.vstack(val_5p_label)
    ...
    for epoch in range(1, N_EPOCH + 1):
        ...
        # acc_src, acc_m, f1score = test(model, [val_5p_data, val_5p_label], epoch)
        # val_cm.write("\n~{}-val_5p\n{}".format(epoch, acc_m))
        # # Validation scalar
        # writer.add_scalar('train/val_5p_acc', acc_src, global_step=epoch)
        # writer.add_text('train/val_5p_f1score', "{}".format(f1score), epoch)
        ...
        if epoch % 10 == 0:
            ...
            test_cm.write("\n~{}\n#{}".format(epoch, acc_m2))
            writer.add_scalar('train/test_acc', acc_src2, epoch)
            writer.add_text('train/test_f1score', "{}".format(f1score), epoch)
...
def test(model, dataloader, epoch, names=None):
    ...
    if names:
        ...
        for name in names:
            ...
            with torch.no_grad():
                for times in range(int(t_imgs.shape[0] / BATCH_SIZE)):
                    ...
                    predict_matrix += confusion_matrix(pred[1].cpu().detach(), label[1].cpu().detach(),
                                                       labels=[0, 1, 2, 3, 4, 5])
        ...
        t_imgs = torch.tensor(dataloader[0]).double().cuda()
        t_lables = torch.tensor(dataloader[1]).double().cuda()
        with torch.no_grad():
            for times in range(int(t_imgs.shape[0] / BATCH_SIZE)):
                t_img, t_lable = t_imgs[BATCH_SIZE * times:BATCH_SIZE * (times + 1)], t_lables[
                                                                                      BATCH_SIZE * times:BATCH_SIZE * (
